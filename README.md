...
1. Preference index of kite-flying location
2. Distance to kite-flying location from base location (residence)
3. Type of kite most suited to wind speed
...
1. The Server accesses darksky.net every hour to obtain the wind speed and wind direction for multiple kite-flying locations that are registered in the database, then stores them in the database.
2.  As defined by Action on Google, the Tako Assistant app analyzes the language from the voice input received through Google Home using Dialogflow, and requests the Server for the necessary information contained in the utterance through Firebase.
3.  The Server receives the necessary information from the database according to the request, then returns the analyzed results to Firebase.
4.  Firebase, having received the necessary information in the utterance from the Server, produces the speech for Google Home.
...
1.  Action on Google account
2.  Paid Firebase plan
3.  DarkSky API (weather information retrieval) account
4.  Web server (compatible with PHP, PostgreSQL, and Cron)
...
Wind direction
0 to 359 degrees, 0 = north, clockwise
...
/takoassistant/load_weather.php
Gets weather information from DarkSky.net and stores it in the database

/takoassistant/search.php
API which receives requests from Firebase

/takoassistant/lib/simple_pg.php
Library to be used by PostgreSQL

/takoassistant/incs/config.inc.php
General configuration

/takoassistant/incs/constant.php
Kite Configuration
